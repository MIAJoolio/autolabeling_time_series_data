...
app.layout = html.Div([
    ...
    # Галочки для выбора характеристик данных
    html.Label("Характеристики данных"),
    dcc.Checklist(
        id='data-characteristics',
        options=[
            {'label': 'Тренд', 'value': 'trend'},
            {'label': 'Сезонность', 'value': 'seasonality'},
    ...
    dash_table.DataTable(
        ...
        columns=[
            ...
            {'name': 'Метод предобработки', 'id': 'preprocessing_method'}  # Добавляем колонку
        ],
        data=[],
    ...
    html.Button('Найти оптимальное решение', id='find-optimal-button', style={'margin': '10px'}),
...
# Callback для обновления списка доступных методов предобработки
@app.callback(
    Output('method-selector', 'options'),
    Input('data-characteristics', 'value')
)
def update_preprocessing_methods(characteristics):
    # Базовые методы для тренда
    methods = []
    ...
    if 'seasonality' in characteristics:
        methods.extend([
    ...
    # Методы для шума
    if 'noise' in characteristics:
        methods.extend([
            {'label': 'DFT Signal', 'value': 'dft_signal'},
            {'label': 'DFT Approximation', 'value': 'dft_approximation'},
            {'label': 'DWT Signal', 'value': 'dwt_signal'}
        ])
    
    # Если ничего не выбрано, доступен только Raw
    if not characteristics:
        methods.append({'label': 'Raw', 'value': 'raw'})
...
# Callback для поиска оптимального решения
@app.callback(
    Output('silhouette-table', 'data', allow_duplicate=True),  # Разрешаем дублирование вывода
    Output('cluster-labels', 'data', allow_duplicate=True),  # Возвращаем метки кластеров
    Input('find-optimal-button', 'n_clicks'),  # Кнопка "Найти оптимальное решение"
    State('stored-data', 'data'),
    State('stored-segments', 'data'),
    State('data-characteristics', 'value'),  # Используем выбранные характеристики данных
    State('clustering-method-selector', 'value'),  # Используем выбранный метод кластеризации
    prevent_initial_call=True
)
def find_optimal_solution(n_clicks, data, segments, characteristics, clustering_method):
    if n_clicks is None:
    ...
    # Получаем список доступных методов предобработки
    preprocessing_methods = update_preprocessing_methods(characteristics)
    
    results = []
    best_score = -1
    best_threshold = max_value / 2  # Начальное значение ползунка
    best_method = 'raw'  # Лучший метод предобработки
    best_labels = []  # Лучшие метки кластеров

    # Перебираем все методы предобработки
    for method in preprocessing_methods:
        ...
        for segment in segments:
            if method_value == 'raw':
                transformed_segments.append(segment)
            elif method_value == 'paa_features':
                transformed_segments.append(paa_features(np.array(segment)))
            elif method_value == 'tsa_detrend':
                transformed_segments.append(tsa_detrend(np.array(segment)))
            elif method_value == 'tsa_acf':
                transformed_segments.append(tsa_acf(np.array(segment)))
            elif method_value == 'statistical_features':
                transformed_segments.append(statistical_features(np.array(segment)))
            elif method_value == 'signal_peaks_features':
                transformed_segments.append(signal_peaks_features(np.array(segment)))
            elif method_value == 'stft_features':
                transformed_segments.append(stft_features(np.array(segment)))
            elif method_value == 'dft_components':
                transformed_segments.append(dft_components(np.array(segment)))
            elif method_value == 'dwt_features':
                transformed_segments.append(dwt_features(np.array(segment)))
            elif method_value == 'dft_signal':
                transformed_segments.append(dft_signal(np.array(segment)))
            elif method_value == 'dft_approximation':
                transformed_segments.append(dft_approximation(np.array(segment)))
        
        data_array = np.array(transformed_segments)
        
        # Перебираем значения distance_threshold
        for threshold in np.arange(1, max_value + 1, (max_value - 1) / 10):  # 10 шагов
            ...
            if len(set(labels)) > 1:  # Silhouette score требует как минимум 2 кластера
                ...
                results.append({
                    'distance_threshold': round(threshold, 1),
                    'silhouette_score': round(score, 3),
                    'preprocessing_method': method_value  # Добавляем метод предобработки
                })
                if score > best_score:
                    best_score = score
                    best_threshold = threshold
                    best_method = method_value
                    best_labels = labels  # Сохраняем лучшие метки
    
    return results, best_labels  # Возвращаем результаты и лучшие метки
...
@app.callback(
    ...
    Input('apply-method-button', 'n_clicks'),
    Input('cluster-button', 'n_clicks'),
    Input('find-optimal-button', 'n_clicks'),
    State('stored-data', 'data'),
    State('stored-segments', 'data'),
    State('transformed-segments', 'data'),
...
def update_graph(show_clicks, reset_clicks, apply_method_clicks, cluster_clicks, find_optimal_clicks, data, segments, transformed_segments, cluster_labels, window_size, method):
    ...
    elif button_id == 'cluster-button' or button_id == 'find-optimal-button':
        fig = go.Figure()
        if not segments or not cluster_labels:
            return go.Figure()
        
        data_to_plot = segments  # Используем исходные данные для отображения
