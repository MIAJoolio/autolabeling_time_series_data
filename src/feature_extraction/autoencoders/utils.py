...
def visualize_latent_space(train_latents, train_labels, val_latents, val_labels, model_save_path):
    ...
    scatter_val = plt.scatter(val_2d[:, 0], val_2d[:, 1], 
                              c=val_labels, cmap='tab10', marker='X', s=100, 
                              label='Val', edgecolor='k')
...
# def visualize_latent_space(latents, labels, model_save_path):
#     """
#     Функция для визуализации полученного латентного пространства
#     """
#     # Обучение t-SNE только на train
#     tsne = TSNE(n_components=2, random_state=42, init='random')
#     train_2d = tsne.fit_transform(latents)  
    
#     # Визуализация
#     plt.figure(figsize=(12, 8))

#     # Точки из train
#     scatter_train = plt.scatter(train_2d[:, 0], train_2d[:, 1], c=labels, cmap='tab10', s=60, label='Train', edgecolor='k', alpha=0.7)
