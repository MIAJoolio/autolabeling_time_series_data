...
def harmonic_oscillator(amplitude: float = 10, frequency: float = 1, damping: float = 0.05, t_peak: float = 20.0, length: int = 100 ) -> np.ndarray:
    """
...
def sawtooth_shift(amplitude: float = 1.0, frequency: float = 1.0, length: int = 101) -> np.ndarray:
    t = np.arange(length)
    period = length / frequency  # длина одного периода
    first_period_mask = t < period  # маска для первого периода
